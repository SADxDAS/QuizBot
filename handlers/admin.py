...
async def cmd_clear_db(message: Message, pool: asyncpg.Pool):
    if message.from_user.id not in config.ADMIN_IDS: return
    async with pool.acquire() as conn:
        await conn.execute('TRUNCATE TABLE answers, questions RESTART IDENTITY CASCADE;')
    config.ACTIVE_QUESTION_ID = None
...
async def cmd_test_data(message: Message, pool: asyncpg.Pool):
    if message.from_user.id not in config.ADMIN_IDS: return
    async with pool.acquire() as conn:
        for i in range(1, 11): await conn.execute('INSERT INTO questions (question_text) VALUES ($1)',
                                                  f'Тестове запитання №{i}')
...
async def activate_question(callback: CallbackQuery, bot: Bot, pool: asyncpg.Pool):
    parts = callback.data.split("_")
    q_id, page = int(parts[1]), int(parts[2]) if len(parts) > 2 else 0

    config.ACTIVE_QUESTION_ID = q_id  # ОНОВЛЮЄМО КЕШ МИТТЄВО!
...
async def stop_question(callback: CallbackQuery, pool: asyncpg.Pool):
    ...
    config.ACTIVE_QUESTION_ID = None  # СКИДАЄМО КЕШ
    async with pool.acquire() as conn:
        await conn.execute('UPDATE questions SET is_active = FALSE')
    await callback.message.edit_reply_markup(reply_markup=await get_toggle_keyboard(pool, page))
...
async def conf_del(callback: CallbackQuery):
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Так, видалити", callback_data=f"delete_q_{callback.data.split('_')[2]}")
    builder.button(text="❌ Скасувати", callback_data="mng_page_0")
    await callback.message.edit_text("⚠️ Ви впевнені, що хочете видалити це питання?", reply_markup=builder.as_markup())


@router.callback_query(F.data.startswith("delete_q_"))
async def del_q(callback: CallbackQuery, pool: asyncpg.Pool):
    async with pool.acquire() as conn:
        await conn.execute('DELETE FROM questions WHERE id = $1', int(callback.data.split('_')[2]))
    await callback.message.edit_text("🗑 Питання успішно видалено. Оновлений список:",
                                     reply_markup=await get_manage_list_keyboard(pool, page=0))
